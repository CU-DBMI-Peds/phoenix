...
################################################################################
# file: tests/test-prepare-immunologic-inputs.R
#
# Phoenix-8 immunologic preparation consists of ANC and ALC.  Both are
# continuous/range-based values and both use the lower value when duplicates are
# present at one encounter time.
################################################################################
...
run_range_case <- function(fun_name,
  ...
  prepared_custom <- lapply(
    X = testdata,
    FUN = function(x) do.call(FUN, list(x = x, id.vars = id.vars, eclock = eclock, value.var = value.var, tie.breaker = max, verbose = FALSE))
  )
...
run_range_case("prepare_anc", "ANC", "anc_value", c(0.8, 0.4, 0.7, 0.6), c(0.4, 0.7, 0.6), c(0.8, 0.7, 0.6), c(0, 5))
run_range_case("prepare_alc", "ALC", "alc_value", c(1.5, 0.9, 1.2, 1.1), c(0.9, 1.2, 1.1), c(1.5, 1.2, 1.1), c(0, 5))
