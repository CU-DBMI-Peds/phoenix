...
eg <-
  lapply(
    X = testdata,
    FUN = function(x) {
      phoenix_coagulation(platelets = platelets, inr = inr, d_dimer = d_dimer, fibrinogen = fibrinogen, data = x)
    }
  )
...
test_empty_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_coagulation(data = x)
  )
...
test_zero_row_result <-
  lapply(
    X = testdata,
    FUN = function(x) {
      phoenix_coagulation(platelets = platelets, inr = inr, d_dimer = d_dimer, fibrinogen = fibrinogen, data = x[0, ])
    }
  )
