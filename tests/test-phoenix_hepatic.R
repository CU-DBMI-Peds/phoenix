...
eg <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_hepatic(bilirubin, alt, data = x)
  )
...
test_empty_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_hepatic(data = x)
  )
...
test_zero_row_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_hepatic(bilirubin, alt, data = x[0, ])
  )
...
test_list_result <- phoenix_hepatic(bilirubin = bilirubin, alt = alt, data = test_list_data)
test_env_result <- phoenix_hepatic(bilirubin = bilirubin, alt = alt, data = test_env_data)
...
test_bad_env_result <- tryCatch(
  phoenix_hepatic(bilirubin = bilirubin, alt = alt, data = test_bad_env_data),
  error = function(e) e
)
