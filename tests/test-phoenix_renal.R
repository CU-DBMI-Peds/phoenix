...
eg <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_renal(creatinine, age, data = x)
  )
...
test_empty_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_renal(data = x)
  )
...
test_zero_row_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_renal(creatinine, age, data = x[0, ])
  )
...
test_list_result <- phoenix_renal(creatinine = creatinine, age = age, data = test_list_data)
test_env_result <- phoenix_renal(creatinine = creatinine, age = age, data = test_env_data)
...
test_bad_env_result <- tryCatch(
  phoenix_renal(creatinine = creatinine, age = age, data = test_bad_env_data),
  error = function(e) e
)
...
stopifnot(
  identical(
    phoenix_renal(creatinine = c(1.0, 1.0), age = c(215.999, 216)),
    c(1L, 0L)
  )
)
