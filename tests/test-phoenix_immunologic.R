...
eg <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_immunologic(anc, alc, x)
  )
...
test_empty_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_immunologic(data = x)
  )
...
test_zero_row_result <-
  lapply(
    X = testdata,
    FUN = function(x) phoenix_immunologic(anc, alc, x[0, ])
  )
...
test_list_result <- phoenix_immunologic(anc = anc, alc = alc, data = test_list_data)
test_env_result <- phoenix_immunologic(anc = anc, alc = alc, data = test_env_data)
...
test_bad_env_result <- tryCatch(
  phoenix_immunologic(anc = anc, alc = alc, data = test_bad_env_data),
  error = function(e) e
)
