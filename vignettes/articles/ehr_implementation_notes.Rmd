...
* Before sending data to the package, verify the data. Some values may provide warnings, others may generate errors. Verify that your code can handle warnings and errors.
* Ideally, both input values and time of the measurement so that last observation carried forward logic can be implemented with limits and conditions for age of inputs for calculated variables can be verified.
