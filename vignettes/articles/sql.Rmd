...
criteria within an SQLite database.  We will use the same example data set
...
We need to either create or write code for the PF ratio, SF ratio, invasive mechanical
...
WITH respiratory AS (
  SELECT
    ...
    COALESCE(vent, 0)          AS invasive_mechanical_ventilation,
    IIF(fio2 > 0.21 OR vent = 1, 1, 0) AS other_respiratory_support
  FROM sepsis
)
SELECT *,
  invasive_mechanical_ventilation * (IIF(pfr < 100 OR sfr < 148, 1, 0) + IIF(pfr < 200 OR sfr < 220, 1, 0) ) +
...
arterial pressure will require both values to be known for possible points to be
assessed.
...
WITH vars AS (
  SELECT
    ...
    lactate,
    dbp + (sbp - dbp) / 3 AS mean_arterial_pressure,
    age
  FROM sepsis
),
points AS (
  SELECT *,
    ...
    CASE WHEN (age >=   0 AND age <    1) AND (mean_arterial_pressure < 17) THEN 2
         WHEN (age >=   1 AND age <   12) AND (mean_arterial_pressure < 25) THEN 2
         WHEN (age >=  12 AND age <   24) AND (mean_arterial_pressure < 31) THEN 2
         WHEN (age >=  24 AND age <   60) AND (mean_arterial_pressure < 32) THEN 2
         WHEN (age >=  60 AND age <  144) AND (mean_arterial_pressure < 36) THEN 2
         WHEN (age >= 144 AND age <= 216) AND (mean_arterial_pressure < 38) THEN 2
         WHEN (age >=   0 AND age <    1) AND (mean_arterial_pressure < 31) THEN 1
         WHEN (age >=   1 AND age <   12) AND (mean_arterial_pressure < 39) THEN 1
         WHEN (age >=  12 AND age <   24) AND (mean_arterial_pressure < 44) THEN 1
         WHEN (age >=  24 AND age <   60) AND (mean_arterial_pressure < 45) THEN 1
         WHEN (age >=  60 AND age <  144) AND (mean_arterial_pressure < 49) THEN 1
         WHEN (age >= 144 AND age <= 216) AND (mean_arterial_pressure < 52) THEN 1
...
For Phoenix 8 you need only four more organ dysfunction scores.  The result
of the following query is saved as an R `data.frame` called `phoenix8_scores`
