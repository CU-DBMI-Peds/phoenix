...
\begin{equation}
  \label{eq:vasos}
  \DS{Vasos}[\m{vaso}] \in \{0,1,2\} \quad \forall m_{vaso},t
\end{equation}
...
\begin{equation}
  \DS{MAP}[\m{bp}, \mathrm{age}] \in \{0, 1, 2\} \quad \forall m_{bp},t.
\end{equation}
...
\begin{equation}
  \DS{Lactate}[\m{lac}] \in \{0, 1, 2\} \quad \forall m_{lac},t.
\end{equation}
...
\begin{equation}
  ...
  \DS{GCS}[\m{gcs}] \coloneqq \mathbb{I}\left\{ GCS\left(t; m_{gcs}\right) \leq 10\right\}
\end{equation}
...
\begin{equation}
  \label{eq:pupils}
  \DS{Pupils}[\m{pupil}] \coloneqq 2 \times \mathbb{I}\left\{\mathcal{C}[Pupils](t;m_{pupil}) = 1\right\}.
\end{equation}
...
\begin{equation}
\begin{aligned}
  \DS{Platelets}[\m{coag}]  &\coloneqq \mathbb{I}\left\{\mathcal{C}[Platelets](t;m_{coag}) < 100\right\} \\
  \DS{INR}[\m{coag}]        &\coloneqq \mathbb{I}\left\{\mathcal{C}[INR](t;m_{coag}) > 1.3\right\} \\
  \DS{DDimer}[\m{coag}]     &\coloneqq \mathbb{I}\left\{\mathcal{C}[DDimer](t;m_{coag}) > 2\right\} \\
  \DS{Fibrinogen}[\m{coag}] &\coloneqq \mathbb{I}\left\{\mathcal{C}[Fibrinogen](t;m_{coag}) < 100\right\},
\end{aligned}
\end{equation}
and then
\begin{equation}
  \DS{Coag}[\m{coag}] \coloneqq \min \left\{2, \sum_{x \in \boldsymbol{C}} \DS{x}[\m{coag}] \right\},
\end{equation}
where
\begin{equation}
...
\begin{multline}
  ...
  \left[\max_{t \in T} \SI \right]
  \Bigg[
    \sum_{x \in \Omega_{d}\setminus \left\{Card\right\}} \max_{t \in T} \DS{x} +
    \sum_{x \in \boldsymbol{C}_{card}} \max_{t \in T} \DS{x}
  \Bigg],
  \quad d \in \left\{4, 8\right\},
\end{multline}
where
...
\begin{equation}
  ...
  \right]
  \left[
    \mathbb{I} \left\{ \sum_{x \in \boldsymbol{C}_{card}}\max_{t \in T} \DS{x} \geq \kappa\right\}
  \right]
\end{equation}
for $\sigma \geq \kappa > 0.$
...
\begin{multline}
    ...
    \shoveleft{ \quad \quad
      ...
      \Bigg[
        ...
        \min \left\{ 2, \sum_{x \in \boldsymbol{C}_{coag}} \max_{t \in T} \DS{x}[\boldsymbol{m}]\right\}
    } \\
...
\begin{equation}
  ...
  \DS{GCS}[\boldsymbol{m}],
  \DS{Pupils}[\boldsymbol{m}]
  \right\}
  =
  \max \left\{
  \max_{t \in T} \DS{GCS}[\boldsymbol{m}],
  \max_{t \in T} \DS{Pupils}[\boldsymbol{m}]
  \right\}
\end{equation}
