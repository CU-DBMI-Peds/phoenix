...
\subsubsection{Coagulation Organ Dysfunction Score}\label{sec:coag-odss}
...
\subsubsection{Endocrine Organ Dysfunction Score}\label{sec:endo-odss}
...
\begin{equation}
  ...
  \SI \coloneqq \maxtinT \left( \mathbb{I}\left\{t \geq t_{1}\right\} \times \mathbb{I}\left\{t \geq t_{2}\right\} \right).
\end{equation}
...
The organ dysfunction summary score (ODSS) summarizes organ dysfunction over $T$
regardless of suspected infection status. This makes ODSS useful as a severity
measure even when a patient does not meet the operational definition of
suspected infection. The Phoenix Sepsis Score (PSS) combines ODSS with evidence
of suspected infection (via SI), so it reflects organ dysfunction in the
presence of suspected infection.

Importantly: The ODSS is a convenience score and has not been evaluated in the
absence of suspected infection; the development of PSS only investigated ODSS
in the presence of suspected infection~\citep{sanchezpinto_2024_development}.
\begin{equation}
  \label{eq:odss}
...
infection.  Specifically, the four-system ODSS of at least $\sigma > 0:$
\begin{equation}
  \label{eq:sepsis}
...
Conceptually, this JAMA ODSS definition is time-aligned: at each time $t$ we sum
the organ dysfunction components evaluated at that same time, then take the
maximum of that sum over $T$. This preserves temporal alignment across organ
systems, so a high score reflects concurrent multi-organ dysfunction at a single
time point.
...
\begin{equation}
  ...
  \ODSS{d}[OLM] \coloneqq
  \sum_{x \in \Omega_{d}} \max_{t \in T} \DS*{x},
...
\begin{equation}
  ...
  \mathbb{I} \left\{ \max_{t \in T} \DS{Card} \geq \kappa\right\}
  \Sepsis[OLM],
\end{equation}
...
and lactate within the cardiovascular system.  Note: vasocatives can be
...
\begin{equation}
  ...
  \ODSS{d}[CCD] \coloneqq \\
  \Bigg[
    \sum_{x \in \Omega_{d}\setminus \left\{\mathrm{Card}\right\}} \max_{t \in T} \DS*{x} +
    \sum_{x \in \Ccard} \max_{t \in T} \DS*{x}
  \Bigg].
\end{equation}
...
\begin{equation}
  \label{eq:pss-ccd}
  % R: score_prepared_phoenix_data(..., aggregation = "ccd"); ccd()
  \PSS{d}[CCD] \coloneqq \SI \ODSS{d}[CCD].
\end{equation}
The definitions for sepsis is structurally the same as the primary and OLM
variants.
\begin{equation}
  \label{eq:odss-ccd-bound}
  \ODSS{d} \leq \ODSS{d}[OLM] \leq \mathrm{ODS}_{d}[CCD].
\end{equation}
...
\begin{equation}
  ...
  \left[ \Sepsis[CCD] \right]
  \mathbb{I} \left\{ \sum_{x \in \Ccard}\max_{t \in T} \DS*{x} \geq \kappa\right\}
\end{equation}
for $\sigma \geq \kappa > 0.$
...
\begin{equation}
  ...
  \begin{split}
    ...
    & \max_{t \in T} \left( \IMV \right) \times \left(\mathbb{I}\{B_3(T; \m{resp})\} + \mathbb{I}\{B_2(T; \m{resp})\}\right) \\
    &+ \max_{t \in T} \left(\ORS\right) \times \mathbb{I}\{B_1(T; \m{resp})\},
  \end{split}
\end{equation}
where
\begin{equation}
...
\begin{aligned}
  B_1(T; \m{resp}) &\coloneqq \min_{t \in T} \PFR < 400 \vee \min_{t \in T} \SFR < 292, \\
  B_2(T; \m{resp}) &\coloneqq \min_{t \in T} \PFR < 200 \vee \min_{t \in T} \SFR < 220, \\
  B_3(T; \m{resp}) &\coloneqq \min_{t \in T} \PFR < 100 \vee \min_{t \in T} \SFR < 148.
\end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \max_{t \in T} \mathbb{I}\left\{\mathcal{C}[v](t;\m{vaso}) = 1\right\}
  \right\}.
\end{equation}
...
\begin{multline}
  ...
  \ODSS{d}[FCD] \coloneqq \\
    \shoveleft{ \quad \quad
      ...
      \min \left\{ 2, \sum_{x \in \Ccoag} \max_{t \in T} \DS*{x}[\boldsymbol{m}]\right\}
      + \sum_{x \in \omega_{d}} \max_{t \in T} \DS*{x}
    \Bigg],
\end{multline}
where
...
\begin{equation}
  ...
  \max_{t \in T} \max \left\{ \DS{GCS}[\m{gcs}], \DS{Pupils}[\m{pupil}] \right\}
  =
  \max \left\{ \max_{t \in T} \DS{GCS}[\m{gcs}], \max_{t \in T} \DS{Pupils}[\m{pupil}] \right\}
\end{equation}
...
\begin{equation}
  \label{eq:pss-fcd}
  % R: score_prepared_phoenix_data(..., aggregation = "fcd"); fcd()
  \PSS{d}[FCD] \coloneqq \SI \ODSS{d}[FCD],
\end{equation}
...
\begin{equation}
  ...
  % R: score_prepared_phoenix_data(..., aggregation = "fcd"); fcd()
  \SepticShock[FCD]\coloneqq
  \Sepsis[FCD]
  \mathbb{I} \left\{ \mathrm{Vasos}^{\mathrm{FCD}}(T;\m{vaso}) \geq \kappa \right\}
\end{equation}
for $\sigma \geq \kappa > 0.$
