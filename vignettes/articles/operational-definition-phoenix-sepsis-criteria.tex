...
\begin{equation}
  ...
  \PSS{d} \coloneqq
  \left[ \max_{t \in T} \SI \right] \left[ \max_{t \in T} \sum_{x \in \Omega_{d}} \DS*{x} \right], \quad d \in \left\{4, 8\right\},
\end{equation}
where
\begin{equation}
  \label{eq:omega4}
...
\begin{equation}
  ...
  \mathbb{I} \left\{
    \max_{t \in T} \SI \times \max_{t \in T} \left( \sum_{x \in \Omega_{4}} \DS*{x}
  \mathbb{I}\left\{\DS{Card} \geq
  \kappa
  \right\} \right)
  \geq \sigma \right\},
\end{equation}
...
\begin{equation}
  ...
  \left[\sum_{x \in \Omega_{d}} \max_{t \in T} \DS*{x}\right],
  \quad d \in \left\{4, 8\right\}.
\end{equation}

\PSS{d}[OLM] decouples time across organ systems. It takes the
...
\PSS{d}[OLM] can be larger than the \PSS{d}, because peaks
from different time points can accumulate. That is, by construction,
\begin{equation}
  \label{eq:pss-olm-bound}
  \PSS{d} \leq \PSS{d}[OLM].
\end{equation}
...
In this exploratory aggregation scheme we decouple all organ systems
...
The constuction here creates the relationsip:
\begin{equation}
  \label{eq:pss-ccd-bound}
  \PSS{d} \leq \PSS{d}[OLM] \leq \PSS{d}[CCD].
\end{equation}
...
and are therefore minimized. For vasoactive medications, each individual
medication indicator is maximized independently before applying the cap used by
the cardiovascular score.
...
The full component decoupling vasoactive contribution is:
\begin{equation}
  \label{eq:vasos-fcd}
  % R: score_prepared_phoenix_data(..., aggregation = "fcd"); fcd()
  \DS{Vasos}^{\mathrm{FCD}}(T;\m{vaso}) \coloneqq
...
The constuction here creates the relationsip:
\begin{equation}
  \label{eq:pss-fcd-bound}
  \PSS{d} \leq \PSS{d}[OLM] \leq \PSS{d}[CCD] \leq \PSS{d}[FCD].
\end{equation}
...
\begin{equation}
  ...
  \left[ \Sepsis[FCD] \right]
  \mathbb{I} \left\{
    \DS{Vasos}^{\mathrm{FCD}}(T;\m{vaso}) +
    \sum_{x \in \boldsymbol{C}_{\mathrm{card}}\setminus\{\mathrm{Vasos}\}}\max_{t \in T} \DS*{x}
    \geq \kappa
  \right\}
\end{equation}
for $\sigma \geq \kappa > 0.$
