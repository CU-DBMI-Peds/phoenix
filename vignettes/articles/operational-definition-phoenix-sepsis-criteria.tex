...
\caption{Notation used in the operational definition of the Phoenix Sepsis Score.}
  ...
  \multicolumn{3}{l}{\textbf{Arithmetic and Functional Operators}} \\
    ...
    returns the smallest value of the set $\boldsymbol{A}$, e.g., $\max\left\{1,
    2, 3\right\} = 3.$\\
  ...
  \multicolumn{3}{l}{\textbf{Observation History and Carry-Forward}} \\
    ...
    $\mathcal{D}\left[x\right]\left(t;\cdot\right)$ &
    For the scoring entity $x,$ $\mathcal{D}\left[x\right]\left(t;\cdot\right)$
    denotes its point contribution at time $t.$  The entity $x$ may be either
    an individual scoring component or an organ system.  Organ-system scores
    are defined by combining their associated component scores. &
    \\
...
\begin{equation}
  PFR\left(t; m_{resp}\right) \coloneqq
    \begin{cases}
...
In general, the SF ratio is defined as \begin{equation}SFR \coloneqq \frac{SpO_2}{FiO_2}.\end{equation}
...
\begin{equation}
  SFR\left(t; m_{resp}\right) \coloneqq
  \begin{cases}
...
\begin{equation}
  IMV\left(t; m_{resp}\right) \coloneqq \mathbb{I}\left\{\bigvee_{i=1}^{4}
...
\begin{equation}
  \label{eq:ors}
  ORS\left(t; m_{resp}\right) \coloneqq \mathbb{I}\left\{ \bigvee_{i=1}^{6} A_{i}\left(t; m_{resp} \right) \right\}
\end{equation}
...
\begin{equation}
  \mathcal{D}[Resp]\left(t;m_{resp}\right) \coloneqq
...
\begin{equation}
  \begin{aligned}
    B_1\left(t; m_{resp}\right) &\coloneqq PFR\left(t; m_{resp}\right) < 400 \vee SFR\left(t; m_{resp}\right) < 292 \\
    B_2\left(t; m_{resp}\right) &\coloneqq PFR\left(t; m_{resp}\right) < 200 \vee SFR\left(t; m_{resp}\right) < 220 \\
    B_3\left(t; m_{resp}\right) &\coloneqq PFR\left(t; m_{resp}\right) < 100 \vee SFR\left(t; m_{resp}\right) < 148.
  \end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \begin{aligned}
      & \exists i^{\ast} \in \left\{1, 2, 3, 4\right\} \quad \text{such that} \\
...
\begin{equation}
  ...
  \mathcal{D}\left[GCS\right]\left(t;\boldsymbol{m}\right) \coloneqq \mathbb{I}\left\{ GCS\left(t; m_{gcs}\right) \leq 10\right\}
\end{equation}
...
\begin{equation}
  ...
  \mathcal{D}[Renal]\left(t; m_{renal}\right) \coloneqq
  \begin{cases}
...
\begin{equation}
SI(t) \coloneqq \mathbb{I}\left\{T_{InfectiousTestOrdered} \leq t\right\} \times \mathbb{I}\left\{T_{AIA} \leq t\right\}.
\end{equation}
...
\begin{equation}
  \boldsymbol{m} \coloneqq \left(m_{resp}, m_{vaso}, m_{bp}, m_{lac}, m_{gcs}, m_{pupil}, m_{coag}, m_{endo}, m_{immu}, m_{hepatic}, m_{renal}\right).
\end{equation}
...
\begin{equation}
  \label{eq:pss}
  PSS_{d}\left(T;\boldsymbol{m}, age \right) \coloneqq
  \left[ \max_{t \in T} SI(t) \right] \left[ \max_{t \in T} \sum_{x \in \Omega_{d}} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right) \right], \quad d \in \left\{4, 8\right\},
\end{equation}
where
\begin{equation}
...
\begin{equation}
  \label{eq:sepsis}
  Sepsis\left(T;\boldsymbol{m},age\right) \coloneqq \mathbb{I}
...
\begin{equation}
  ...
  SepticShock\left(T;\boldsymbol{m},age\right) \coloneqq
  \mathbb{I} \left\{
...
\begin{longtable}{lrl}
  ...
  \rowcolor{gray!20}
    ~~$m_{renal}$  & 1440 minutes & creatine
\end{longtable}
...
\begin{equation}
  \label{eq:pss-olm}
  \substack{PSS_{d} \\ OLM}\left(T;\boldsymbol{m}, age \right) \coloneqq
  \left[\max_{t \in T} SI(t) \right]
  \left[\sum_{x \in \Omega_{d}} \max_{t \in T} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right)\right],
  \quad d \in \left\{4, 8\right\}.
\end{equation}

$\substack{PSS_{d} \\ OLM}$ decouples time across organ systems. It takes the
...
$\substack{PSS_{d} \\ OLM}$ can be larger than the $PSS_{d}$, because peaks
from different time points can accumulate. That is, by construction,
\begin{equation}
  PSS_{d}\left(T; \boldsymbol{m}, age\right) \leq \substack{PSS_{d} \\ OLM}\left(T; \boldsymbol{m}, age\right).
\end{equation}
...
Define
\begin{equation}
  \Omega_{d}^{\prime} \coloneqq \Omega_{d}\backslash \left\{Card\right\} \cup \left\{Vasos, MAP, Lactate\right\}.
\end{equation}
Then
\begin{equation}
  \substack{PSS_{d} \\ CCD} \coloneqq
  \left[\max_{t \in T} SI(t) \right]
  \left[\sum_{x \in \Omega_{d}^{\prime}} \max_{t \in T} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right)\right],
  \quad d \in \left\{4, 8\right\}
\end{equation}
...
\begin{aligned}
  B_1(t; m_{resp}) &\coloneqq \min_{t \in T} PFR(t; m_{resp}) < 400 \vee \min_{t \in T} SFR(t; m_{resp}) < 292, \\
  B_2(t; m_{resp}) &\coloneqq \min_{t \in T} PFR(t; m_{resp}) < 200 \vee \min_{t \in T} SFR(t; m_{resp}) < 220, \\
  B_3(t; m_{resp}) &\coloneqq \min_{t \in T} PFR(t; m_{resp}) < 100 \vee \min_{t \in T} SFR(t; m_{resp}) < 148.
\end{aligned}
\end{equation}
The overall scores are then defined as:
\begin{equation}
    \substack{PSS_{d} \\ FCD}\left(T, \boldsymbol{m}, age\right) \coloneqq
     \substack{Resp \\ FCD}\left(T; m_{resp}\right)
     + \min \left\{ 2, \sum_{x \in \boldsymbol{C}} \max_{t \in T} \mathcal{D}\left[x\right]\left(t;\boldsymbol{m}\right)\right\}
     + \sum_{x \in \omega_{d}} \max_{t \in T} \mathcal{D}\left[x\right]\left(t; \boldsymbol{m}, age\right),
    \quad d \in \left\{4, 8\right\},
\end{equation}
where
\begin{equation}
