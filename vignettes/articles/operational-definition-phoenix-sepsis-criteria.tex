...
\caption{Notation used in the operational definition of the Phoenix Sepsis Score.}
  ...
  \multicolumn{3}{l}{\textbf{Time and Indexing}} \\
    ...
    $\boldsymbol{m}:=(m_{resp},m_{vaso},m_{map},m_{lac},m_{gcs},m_{pupil},m_{coag},m_{endo},m_{immu},m_{hepatic},m_{renal})$. \\
...
\begin{equation}
  ...
  \begin{cases}
    ...
    %%% \widehat{MAP_{i^{*}}}(t; m_{map}) & \exists i^{*} \quad \text{such that} \\ & i^{*} := \min \left\{i \in \left\{1, 2, 3, 4\right\}: \widehat{MAP_{i}}(t; m_{map}) \ne \mathrm{NA} \right\} \\
    \widehat{MAP_{i^{\ast}}}(t; m_{map}) &
    \begin{aligned}
      & \exists i^{\ast} \in \left\{1, 2, 3, 4\right\} \quad \text{such that} \\ & i^{\ast} := \min \left\{i \in \left\{1, 2, 3, 4\right\}: \widehat{MAP_{i}}(t; m_{map}) \ne \mathrm{NA} \right\} \\
    \end{aligned} \\
    \mathrm{NA} & \text{otherwise.}
  \end{cases}
\end{equation}
...
\begin{equation}
  \mathcal{D}\left[Coag\right](t; m_{coag}) := \min \left(2, \sum_{i=1}^{4} \mathbb{I}\left\{A_{i}(t; m_{coag})\right\} \right)
\end{equation}
where
\begin{equation}
\begin{aligned}
...
\begin{equation}
  \boldsymbol{m} := \left(m_{resp}, m_{vaso}, m_{map}, m_{lac}, m_{gcs}, m_{pupil}, m_{coag}, m_{endo}, m_{immu}, m_{hepatic}, m_{renal}\right).
\end{equation}
...
\begin{equation}
  \label{eq:pss}
  PSS_{k}\left(T;\boldsymbol{m}, age \right) :=
  \left[ \max_{t \in T} SI(t) \right] \left[ \max_{t \in T} \sum_{x \in \Omega_{k}} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right) \right], \quad k \in \left\{4, 8\right\},
\end{equation}
where
\begin{equation}
  \Omega_{4} := \{Resp, Card, Neuro, Coag\}
\end{equation}
and
\begin{equation}
  \Omega_{8} := \Omega_{4} \cup \{Endo, Immu, Hepatic, Renal\}.
\end{equation}

Sepsis is defined as organ-dysfunction in the presence of a suspected infection.  Specifically, a score of at least $\sigma > 0$ points:
\begin{equation}
  \label{eq:sepsis}
  Sepsis\left(T;\boldsymbol{m},age\right) := \mathbb{I}
  \left\{PSS_{4}\left(T;\boldsymbol{m}, age \right) \geq \sigma \right\}.
\end{equation}
...
\begin{equation}
  ...
  \mathbb{I} \left\{
    \max_{t \in T} SI(t) \times \max_{t \in T} \left( \sum_{x \in \Omega_{4}} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right)
...
\begin{itemize}
  ...
  \item Look backs:
    ...
    \begin{tabular}{lrrl}
    \rowcolor{gray!20}
      $m_{resp}$  &  360 minutes &  (6 hours) & all respiratory EHR variables \\
      $m_{vaso}$  &  720 minutes & (12 hours) & vasoactive medications \\
    \rowcolor{gray!20}
      $m_{map}$   &  360 minutes &  (6 hours) & blood pressure / MAP \\
      $m_{lac}$   &  360 minutes &  (6 hours) & lactate \\
    \rowcolor{gray!20}
      $m_{gcs}$   &  720 minutes & (12 hours) & GCS components or total \\
      $m_{pupil}$ &  720 minutes & (12 hours) & pupils \\
    \rowcolor{gray!20}
      $m_{coag}$  & 1440 minutes & (24 hours) & coagulation labs \\
      $m_{endo}$  &  720 minutes & (12 hours) & glucose \\
    \rowcolor{gray!20}
      $m_{immu}$  & 1440 minutes & (24 hours) & ANC and ALC \\
      $m_{hepatic}$  & 1440 minutes & (24 hours) & ALT and Total bilirubin \\
    \rowcolor{gray!20}
      $m_{renal}$  & 1440 minutes & (24 hours) & creatine
    \end{tabular}
...
\section{Exploratory Aggregation Schema}\label{sec:eas}

There are several expiratory aggregation schema. Daily and block‑based ``worst
...
\begin{equation}
  \label{eq:pss-eas1}
  \substack{PSS \\ eas1}\left(T;\boldsymbol{m}, age \right) :=
  \max_{t \in T} SI(t) \times \sum_{x \in \Omega} \max_{t \in T} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right).
\end{equation}

$\substack{PSS \\ eas1}$ decouples time across organ systems. It
...
a result, $\substack{PSS \\ eas1}$ can be larger than the $PSS$, because peaks
from different time points can accumulate.
That is, by construction,
\begin{equation}
PSS\left(T; \boldsymbol{m}, age\right) \leq \substack{PSS \\ eas1}\left(T; \boldsymbol{m}, age\right).
\end{equation}
This is an example of the triangle inequality.

Using \eqref{eq:pss-eas1} to define sepsis is not structurally different from the
primary definition \eqref{eq:sepsis}.
\begin{equation}
  \label{eq:sepsis-eas1}
  \substack{Sepsis \\ eas1}\left(T;\boldsymbol{m},age\right) := \mathbb{I}
  \left\{\substack{PSS \\ eas1}\left(T;\boldsymbol{m}, age \right) \geq \sigma \right\}.
\end{equation}
Septic Shock does require a slight structural difference from the primary
definition \eqref{eq:septicshock}.
\begin{equation}
  \label{eq:septicshock-eas1}
  \substack{SepticShock \\ eas1}\left(T;\boldsymbol{m},age\right) :=
  \mathbb{I} \left\{ \left(
...
Define
\begin{equation}
  \mathcal{D}\left[Card_{eas2}\right]\left(T; m_{vaso}, m_{map}, m_{lac}, age\right) :=
...
\begin{equation}
  \label{eq:pss-eas2}
  \substack{PSS \\ eas2}\left(T; \boldsymbol{m}, age\right) :=
  \max_{t \in T} SI(t) \times \sum_{x \in \Omega_{eas2}} \max_{t \in T} \mathcal{D}[x]\left(t;\boldsymbol{m};age\right)
\end{equation}
where
\begin{equation}
  \Omega_{eas2} := \{Resp, Card_{eas2}, Neuro, Coag\}.
\end{equation}
...
\begin{equation}
Card_{eas3}(T; m_{vaso}, m_{map}, m_{lac}, age) := Card_{eas2}(T; m_{vaso}, m_{map}, m_{lac}, age).
\end{equation}

\noindent Neurological:
\begin{equation}
  Neuro_{eas3}(T; m_{gcs}, m_{pupil}) := \min\left(2,
...
\begin{equation}
  \label{eq:pss-eas3}
  \substack{PSS \\ eas3}\left(T; \boldsymbol{m}, age\right) :=
  \max_{t \in T} SI(t) \times \sum_{x \in \Omega_{eas3}} \max_{t \in T}
  \mathcal{D}[x]\left(t;\boldsymbol{m};age\right),
\end{equation}
where
\begin{equation}
  \Omega_{eas3} := \{Resp_{eas3}, Card_{eas3}, Neuro_{eas3}, Coag_{eas3}\}.
\end{equation}
