...
the provided example synthetic dataset `sepsis`. This data set has
...
### Inputs:
* `pf_ratio`: is the ratio of PaO<sub>2</sub> (partial pressure of oxygen in
...
Another example using the provided example dataset `sepsis`. Here we have to
...
* `mean_arterial_pressure`: mean arterial pressure (mmHg).  During development
  ...
  (SBP - DBP) where DBP is diastolic blood pressure (mmHg) and SBP is systemic
  blood pressure (mmHg).
...
Note: the mean arterial pressure can be approximated by 2/3*DBP + 1/3*SBP. To
...
* `inr`: international normalized ratio; a metric for time require for blood to
  clot.
...
After an hour, she becomes hypotensive again and she is started on a
...
and &geq;1 cardiovascular points, so she meets criteria for septic shock.
...
to be admitted, his level of consciousness deteriorates rapidly: with nailbed
